...
def main():
    ...
    if not os.path.isdir(args.in_dir):
        raise ValueError(f"Input directory not found: {args.in_dir}. Skipping subject.")
    ...
    all_possibles_signatures = np.loadtxt(args.in_assignement_files[0]).astype(np.uint8)
    print(f"Loaded {len(all_possibles_signatures)} signatures from {args.in_assignement_files[0]}")
    ...
    nufo_data = nufo_img.get_fdata().astype(np.float32)
    ...
    tdi_data = np.zeros(wm_data.shape + (max_label,), dtype=np.float32)
    ...
    # Normalize TDI per voxel
    intersection_mask = np.sum(tdi_data, axis=-1) > 0 & wm_mask
    tdi_data[intersection_mask] = tdi_data[intersection_mask] / np.max(tdi_data[intersection_mask], axis=-1, keepdims=True)
    tdi_data = np.clip(tdi_data, 0, 1)

    nufo_data[intersection_mask] = nufo_data[intersection_mask] / np.max(nufo_data[intersection_mask], axis=-1, keepdims=True)
    nufo_data = np.clip(nufo_data, 0, 1)
    ...
    coords = np.array(np.argwhere(intersection_mask)).astype(float).T
    coords /= np.max(coords, axis=1, keepdims=True)
    coords = np.clip(coords, 0, 1)

    # Process each voxel's signature
    # Concatenate nufo and position to signature
    new_signatures = np.insert(tdi_data, 0, nufo_data, axis=1)
    new_signatures = np.insert(new_signatures, 0, coords, axis=1)

    MAX_LABELS = 50
    kmeans = KMeans(n_clusters=MAX_LABELS, init='random', n_init=10,
                    random_state=0, verbose=1)
    kmeans.fit(new_signatures)
    labels_ravel = kmeans.labels_
    labels_ravel = np.clip(labels_ravel, 0, MAX_LABELS).astype(np.uint16)
    ...
    nib.save(nib.Nifti1Image(labels.astype(np.uint16), wm_img.affine), args.out_labels)
